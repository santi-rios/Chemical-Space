...
read.csv("./data6/df_cc_3.csv") |>
  ...
  # Group by the color group so we can assign a rank for variations
  dplyr::mutate(
  ...
  dplyr::ungroup() |>
  # Now assign colours using a numeric scale for blue and red groups
  dplyr::mutate(
    cc = case_when(
      color_group == "blue" ~ scales::col_numeric(
        palette = c("#60b1ff", "#00264c"), # lighter to darker blues
        domain = c(1, max_rank)
      )(iso_rank),
      color_group == "red" ~ scales::col_numeric(
        palette = c("#ff1d1d", "#330000"), # lighter to darker reds
        domain = c(1, max_rank)
      )(iso_rank),
      TRUE ~ cc
    )
