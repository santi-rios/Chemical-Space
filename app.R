...
server <- function(input, output, session) {
  ...
  collaboration_data <- reactive({
    req(input$country_select, input$years, input$collab_types, input$chemical_category, input$data_type)

    # Validate collab_types input
    if (length(input$collab_types) == 0 && input$data_type != "individual") {
    ...
    bindCache(input$country_select, input$years, input$collab_types, input$chemical_category, input$data_type) %>%
    bindEvent(input$country_select, input$years, input$collab_types, input$chemical_category, input$data_type)
