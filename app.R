...
server <- function(input, output, session) {
  message("--- Server function started ---") # Log server start

  # --- Reactive Values ---
  selected_countries_immediate <- reactiveVal(c())
  selected_countries <- debounce(selected_countries_immediate, 1000)
  display_mode <- reactiveVal("compare_individuals")

  # --- Constants ---
  available_color <- "lightgray"

  # --- Prepare Map Data (sf object) ---
  message("Preparing initial map data...") # Log map data prep start
  ...
  map_data <- world_map_prep %>%
      ...
      mutate(
          ...
          cc = ifelse(is.na(cc) | cc == "", "#808080", cc)
      )
  rm(world_map_prep)
  message("...Initial map data prepared.") # Log map data prep end

  # --- Map Interaction ---

  # Render initial map
  output$selection_map <- renderLeaflet({
    message("Rendering initial selection_map...") # Log map render start
    map_output <- create_selection_map(selected_countries_immediate(), country_list, regions)
    message("...Initial selection_map rendering complete.") # Log map render end
    map_output
  })
  ...
  observeEvent(input$selection_map_shape_click, {
    ...
    # message(paste("Map shape clicked:", clicked_iso)) # Optional: Log clicks
    req(clicked_iso)
  ...
  observe({
    ...
    # message(paste("Region filter changed:", current_region_filter)) # Optional: Log filter changes
  ...
  observeEvent(input$clear_selection, {
    message("Clear selection button clicked.") # Log clear selection
  ...
  output$region_filter_ui <- renderUI({
    # message("Rendering region_filter_ui...") # Usually less critical unless complex
    tagList(
      selectInput(
        "region_filter", "Filter Map Layers by Region:",
  ...
  output$display_mode_ui <- renderUI({
    ...
    if (length(countries) > 1) {
      # message("Rendering display_mode_ui...") # Log UI render
      radioButtons(
  ...
  observeEvent(input$display_mode_select, {
    message(paste("Display mode selected:", input$display_mode_select)) # Log mode change
  ...
  output$plot_header_ui <- renderUI({
    ...
    # message("Rendering plot_header_ui...") # Log UI render
    get_plot_header(countries, mode, chem, country_list)
  })
  ...
  processed_data <- reactive({
    ...
    message("--- Calculating processed_data START ---") # Log data processing start
    message(paste("  Selected countries:", paste(countries, collapse=", ")))
    message(paste("  Year range:", paste(input$years, collapse="-")))
    message(paste("  Chemical category:", input$chemical_category))
    message(paste("  Display mode:", mode))
    message(paste("  Region filter for data:", current_region_filter_for_data))

    # Use withProgress locally, but rely on logs for server debugging
    # withProgress(message = "Processing data...", { ... })

    data_out <- get_display_data(
        ds = ds,
    ...
    message(paste("--- Calculating processed_data END. Rows:", nrow(data_out), "---")) # Log data processing end
    data_out
  ...
  output$main_plot <- renderPlotly({
    message("Rendering main_plot...") # Log plot render start
    validate(
    ...
    plot_out <- create_main_plot(
      data = data_to_plot,
      display_mode = mode,
      selected_isos = selected_countries(),
      country_list = country_list
    )
    message("...main_plot rendering complete.") # Log plot render end
    plot_out
  })

  output$contributionMapPlot <- renderPlotly({
    message("Rendering contributionMapPlot...") # Log plot render start
    countries <- selected_countries()
    req(countries)
    mode <- if (length(countries) == 1) "individual" else display_mode()
    validate(
    ...
    plot_out <- create_contribution_map_plot(
    ...
    message("...contributionMapPlot rendering complete.") # Log plot render end
    plot_out
  })

  output$summary_table <- renderDT({
    message("Rendering summary_table...") # Log table render start
    countries <- selected_countries()
    ...
    mode <- if (length(selected_countries()) == 1) "individual" else display_mode()
    table_out <- create_summary_table(
      data = data_to_summarize,
      display_mode = mode
    )
    message("...summary_table rendering complete.") # Log table render end
    table_out
  })

  output$summary_value_boxes_ui <- renderUI({
    message("Rendering summary_value_boxes_ui...") # Log UI render start
    chemical_category <- input$chemical_category
    region_filter <- input$region_filter
    req(chemical_category, region_filter)

    top_data_raw <- calculate_top_contributors(
      ds = ds,
      year_range = c(min_year_data, max_year_data),
    ...
    ui_out <- value_box(
      title = paste("Historical Top", nrow(top_data), "Countries (Avg % contribution) for current filters"),
      ...
      tooltip(
        ...
        paste("Showing top", nrow(top_data), "countries based on overall average percentage across all years",
              ...
              "Year range slider is ignored for this box.")
      ),
      style = "white-space: pre-wrap;"
    )
    message("...summary_value_boxes_ui rendering complete.") # Log UI render end
    ui_out
  })

  # --- Article Figure Plots ---
  # Add logs similarly if needed, e.g.:
  output$countrycsPlot <- renderPlotly({
    message("Rendering article: countrycsPlot...")
  ...
  output$articleGdpPlot <- renderPlotly({
    message("Rendering article: articleGdpPlot...")
    req(nrow(article_data) > 0)
  ...
  output$articleResearchersPlot <- renderPlotly({
    message("Rendering article: articleResearchersPlot...")
    req(nrow(article_data) > 0)
  ...
  output$articleCsExpansionPlot <- renderPlotly({
    message("Rendering article: articleCsExpansionPlot...")
    req(nrow(article_data) > 0)
  ...
  output$articleTopCollabsPlot <- renderPlotly({
     message("Rendering article: articleTopCollabsPlot...")
  ...
  message("--- Server function setup complete ---") # Log end of server setup
